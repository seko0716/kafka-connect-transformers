...
public class GroovySeTransformer<R extends ConnectRecord<R>> extends AbstractScriptTransformer<R> {
    private static final Logger log = LoggerFactory.getLogger(GroovySeTransformer.class);
    private Invocable inv;

    @Override
    public Map<String, Object> transform(Map<String, Object> source, String script) {
        if (this.keyScript != null) {
            return groovyTransform(source, "keyTransform");
        }
        if (this.valueScript != null) {
            return groovyTransform(source, "valueTransform");
        }
        return source;
    }

    private Map<String, Object> groovyTransform(Map<String, Object> source, String methodName) {
        try {
